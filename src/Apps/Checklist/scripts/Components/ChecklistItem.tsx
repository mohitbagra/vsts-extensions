        ...
        return (
            <div className="checklist-item" key={checklistItem.id}>
                ...
                <div className={css("checklist-item-label", { checked: isCompleted })} onClick={this._onLabelClick} style={labelStyle}>
                    <TooltipHost content={checklistItem.text} delay={TooltipDelay.medium} overflowMode={TooltipOverflowMode.Parent} directionalHint={DirectionalHint.topCenter}>
                        {checklistItem.text}
                    </TooltipHost>
                </div>

                {isDefaultItem &&
                    !allowEditDefaultItems && (
                        <TooltipHost
                        ...
                        >
                            <IconButton className="checklist-item-button info-button" disabled={disabled} iconProps={{ iconName: "Info" }} />
                        </TooltipHost>
                    )}
                {(!isDefaultItem || allowEditDefaultItems) && (
                    <TooltipHost content={"Edit item"} delay={TooltipDelay.medium} directionalHint={DirectionalHint.bottomRightEdge}>
                        <IconButton className="checklist-item-button edit-item-button" disabled={disabled} iconProps={{ iconName: "Edit" }} onClick={this._onEditItemButtonClick} />
                    </TooltipHost>
                )}
                {(!isDefaultItem || allowEditDefaultItems) && (
                    <TooltipHost content={"Delete item"} delay={TooltipDelay.medium} directionalHint={DirectionalHint.bottomRightEdge}>
                        <IconButton

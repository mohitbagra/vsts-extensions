...
export interface IFontNamePickerProps {
    menuTargetElement: HTMLElement;
    selectedName?: string;
    onDismissMenu(): void;
    onSelectName(value: IFont): void;
}

export class FontNamePicker extends React.Component<IFontNamePickerProps, {}> {
    public render() {
        const { onDismissMenu, menuTargetElement } = this.props;
        let selectedName = this.props.selectedName;

        const fontMatches = (selectedName || "").match(FIRSTFONT_REGEX);
        ...
        return (
            <ContextualMenu
                ...
                items={this._createMenuItems(selectedName)}
            />
        );
    }

    private _createMenuItems(selectedFont: string): IContextualMenuItem[] {
        let dividerKey = 0;
